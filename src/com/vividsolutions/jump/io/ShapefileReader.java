 ...
 * <p>
 * DataProperties for the JUMPReader load(DataProperties) interface:<br>
 * <br>
 * </p>
 * <p>
 ...
 * </tr>
 * <tr>
 * <td colspan='2'> NOTE: The input .dbf is assumed to be 'beside' (in the same
 * directory) as the .shp file. </td>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td colspan='2'> Uses a modified version of geotools to do the .dbf and .shp
 * file reading. If you are reading from a .zip file, the dbf file will be
 * copied to your temp directory and deleted after being read. </td>
 * </tr>
 * </table>
 */
  ...
  public FeatureCollection read(DriverProperties dp)
    ...
    if (shpfileName == null) {
      throw new IllegalParametersException(
        I18N.get("io.ShapefileReader.no-file-property-specified"));
    }

    int loc = shpfileName.lastIndexOf(File.separatorChar);
    String path = shpfileName.substring(0, loc + 1); // ie. "/data1/hills.shp"
                                                      // -> "/data1/"
    String fname = shpfileName.substring(loc + 1); // ie. "/data1/hills.shp" ->
                                                    // "hills.shp"
    ...
    if (loc == -1) {
      throw new IllegalParametersException(
        I18N.get("io.ShapefileReader.filename-must-end-in-shp"));
    }

    String fnameWithoutExtention = fname.substring(0, loc); // ie. "hills.shp"
                                                            // -> "hills"
    ...
    DbfFile mydbf = getDbfFile(dbfFileName,
      dp.getProperty(COMPRESSED_FILE_PROPERTY_KEY));
    ...
    try {
      ...
      myshape.close(); // ensure we can delete input shape files before task is
                        // closed
    }
